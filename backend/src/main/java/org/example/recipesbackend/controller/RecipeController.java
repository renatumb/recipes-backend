...
public class RecipeController {
    ...
    public ResponseEntity<Recipe> createRecipe(@RequestBody Recipe recipe, @PathVariable Long userId) {
        return ResponseEntity.ok().body(recipeService.createRecipe(recipe, userService.getUserById(userId)));
    }
    ...
    @ApiResponses({@ApiResponse(responseCode = "200", description = "Paged available recipes")})
    @GetMapping
    ...
    public ResponseEntity<Recipe> likeRecipe(@PathVariable(name = "recipeID") Long recipeId, @RequestBody Long userId) {
        return ResponseEntity.ok(recipeService.likeRecipe(recipeId, userService.getUserById(userId)));
    }
}
